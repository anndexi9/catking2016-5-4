...
<h3> ${mem.username}様, 新規会員登録成功</h3>
ご登録した会員情報は下記の通りです。
<table>
<tr><td>会員番号</td><td>${mem.code}</td></tr>
<tr><td>ユーザネーム</td><td>${mem.username}</td></tr>
<tr><td>メール</td><td>${mem.email}</td></tr>
<tr><td>本名</td><td>${mem.name}</td></tr>
<tr><td>電話番号</td><td>${mem.tel}</td></tr>
<tr><td>住所</td><td>${mem.address}</td></tr>
<tr><td>登録日時</td><td>${mem.regtime}</td></tr>
</table>
<br>
<form action="/catking/LoginServlet?action=logout" method="post">
